...
- **Subagents (D22):** for a file with main-conversation traffic, version 1
  evaluates `promptCacheTtl` only; a subagent transcript uploaded on its own is
  headlined by its own bucket and fully analyzed. When a file
...
| D27 | Cloudflare **Web Analytics RUM auto-injection is to be turned off** for the `cacheanalyzer.com` zone rather than added to `script-src`, and the data-policy copy now claims what can *run*, not only what we ship | Found in WP-10's production verification (2026-08-30), and only there: Cloudflare injects `static.cloudflareinsights.com/beacon.min.js` into **browser** responses at the edge, after our build — so `curl`, `dist/index.html` and every `workers.dev` preview are clean, and preview-based testing could never have caught it. The CSP blocked it, which is the system working, but it produced four console errors per page load and contradicted the data policy's "no third-party embed of any kind" for every real visitor. Allowing the host would weaken the exact CSP the policy points at as its proof, for analytics the project does not want. The zone setting is a dashboard change (the deploy token has no analytics scope), owned by the user; the copy change ships now and stays accurate either way. The copy is scoped to *third-party* scripts on purpose: `script-src 'self'` blocks another domain's script and every inline script, but would not block one served from our own origin, and this page is the wrong place to overstate a header. **Pending: disable Web Analytics automatic setup for the zone.** (User, 2026-08-30) |
