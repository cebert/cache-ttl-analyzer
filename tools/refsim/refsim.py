...
def session_shape(requests: list[dict], threads: dict[str, list[dict]]) -> dict:
    ...
    for thread in threads.values():
        for i in range(1, len(thread)):
            ...
            # Same boundaries as the expiry test in replay_thread, so the
            # histogram cannot disagree with the simulation it explains.
            if gap <= TTL_MS["5m"]:
                under_5m += 1
            elif gap <= TTL_MS["1h"]:
                in_band += 1
...
def analyze_bucket(bucket: str, requests: list[dict], pricing: dict) -> dict:
    ...
    bucket_tokens = 0
    unpriced_tokens = 0
    # Observed token totals count every request, priced or not: a token count
    # is fact and needs no rate (contract, TokenTotals).
    actual_usage = {
        "inputTokens": 0,
        "cacheReadTokens": 0,
        "cacheWriteTokens": 0,
    ...
    warm_read_requests = 0
    for r in requests:
        usage = r["usage"]
        actual_usage["inputTokens"] += usage["inputTokens"]
        actual_usage["cacheReadTokens"] += usage["cacheReadInputTokens"]
        actual_usage["cacheWriteTokens"] += usage["cacheCreationInputTokens"]
        actual_usage["outputTokens"] += usage["outputTokens"]
        if usage["cacheReadInputTokens"] > 0:
            warm_read_requests += 1
    ...
    analysis = {
        ...
        "warmReadRequestCount": warm_read_requests,
