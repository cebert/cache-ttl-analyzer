...
### `[sent mid-turn]` markers

A message typed while Claude is still working is not recorded as an ordinary
turn — Claude Code logs it as a `queue-operation` — and the renderer keeps only
`user` and `assistant` records, so those messages used to vanish from the HTML
while remaining in `session.jsonl`. Across the first fourteen sessions that hid
**50 human messages**; one session published 2 prompts where 10 were sent.

`publish-transcript` now recovers them, prefixed **`[sent mid-turn]`**. The
prefix matters: the renderer begins a new conversation at every user message,
so a recovered one is displayed as though it were a top-level prompt when it
actually interrupted the turn above it. The committed `session.jsonl` is
untouched by this — it stays the authentic log, and only the HTML gains the
messages.
...
| 2026-08-30 | [011-wp-06-fixtures](011-wp-06-fixtures/) | Completed WP-06 (PR #17): rewrote the feasibility prototype as an independent Python reference simulator, built the golden-fixture harness, synthetic and adversarial fixtures, and scrubbed real captures — including Claude-enacted 5m/1h scenario sessions that became the bundled samples and surfaced a live partial-invalidation case the simulator doesn't yet model | [open](https://cebert.github.io/cache-ttl-analyzer/011-wp-06-fixtures/) | Redacted: user email and name, an unrelated project name from a session-log inventory; home paths and the username rewritten to `~` / `user`. Log ends before the final transcript-publishing turns |
| 2026-08-30 | [012-partial-lapses-and-results-ui](012-partial-lapses-and-results-ui/) | Modelled partially lapsed cache entries (PR #24) — the finding WP-06 pinned but could not yet fix, which flips a real 5m-configured session to 1h — then completed WP-08 (PR #25): the results and insights view the whole tool exists to show | [open](https://cebert.github.io/cache-ttl-analyzer/012-partial-lapses-and-results-ui/) | Redacted: three unrelated project names from a directory listing; home paths and the username rewritten to `~` / `user`. User email kept (already the commit author throughout this repo's history). Log ends before the final transcript-publishing turns |
| 2026-08-30 | [013-ux-polish](013-ux-polish/) | Fixed the UX problems found on the deployed site (PR #26): an off-palette favicon, no way back from an opened session, find-your-logs instructions that interleaved three platforms and buried the keystroke that matters, and the same locations repeated on the landing page. Also built a results view in parallel with PR #25, which landed first and was kept | [open](https://cebert.github.io/cache-ttl-analyzer/013-ux-polish/) | Redacted: user email; home paths rewritten to `~`. Eight mid-turn steering messages are now recovered and marked `[sent mid-turn]`, taking the page from 2 rendered prompts to 10. Log ends before the final turns |
| 2026-08-30 | [014-wp-10-launch-polish](014-wp-10-launch-polish/) | Completed WP-10 (PR #27) and took the MVP to done: fixed a class of i18n plural bug the catalog could not express and added invariant tests that fail the whole shape, rewrote the README to lead with what the tool answers, and verified the launch claims against the deployed preview rather than assuming them — four samples, deep links, CSP, the 100MB fixture at ~1s, and a cancel proven to terminate the worker, in Chromium, Firefox and WebKit | [open](https://cebert.github.io/cache-ttl-analyzer/014-wp-10-launch-polish/) | Redacted: 12 unrelated project names from an `ls` of the parent directory; home paths rewritten to `~`. User email kept (already the commit author throughout this repo's history). Eleven mid-turn steering messages are recovered and marked `[sent mid-turn]`, taking the page from 11 rendered prompts to 22. Log ends before the final turns |
