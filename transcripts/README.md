...
| 2026-08-30 | [013-ux-polish](013-ux-polish/) | Fixed the UX problems found on the deployed site (PR #26): an off-palette favicon, no way back from an opened session, find-your-logs instructions that interleaved three platforms and buried the keystroke that matters, and the same locations repeated on the landing page. Also built a results view in parallel with PR #25, which landed first and was kept | [open](https://cebert.github.io/cache-ttl-analyzer/013-ux-polish/) | Redacted: user email; home paths rewritten to `~`. The rendered HTML shows only the two top-level prompts — seven mid-turn steering messages are recorded as `queue-operation` rows, which the renderer skips; they are in `session.jsonl`. Log ends before the final turns |
| 2026-08-30 | [014-wp-10-launch-polish](014-wp-10-launch-polish/) | Completed WP-10 (PR #27) and took the MVP to done: fixed a class of i18n plural bug the catalog could not express and added invariant tests that fail the whole shape, rewrote the README to lead with what the tool answers, and verified the launch claims against the deployed preview rather than assuming them — four samples, deep links, CSP, the 100MB fixture at ~1s, and a cancel proven to terminate the worker, in Chromium, Firefox and WebKit | [open](https://cebert.github.io/cache-ttl-analyzer/014-wp-10-launch-polish/) | Redacted: 12 unrelated project names from an `ls` of the parent directory; home paths rewritten to `~`. User email kept (already the commit author throughout this repo's history). Only 11 of ~38 human messages render — this session steered heavily mid-turn, and those are `queue-operation` rows the renderer skips; they are all in `session.jsonl`. Log ends before the final turns |
